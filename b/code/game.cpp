...
{
    Foo *fooBar;
    void Initialize(void *memory, MemoryMarker size);
    void Reload();
...
static RCDrawQuad *PushTile(RenderGroup *rg, NavTile *tile)
{
    ...
    case BLOCKING:
    {
        drawTileCmd->m_color = TILE_OCCUPIED_COLOR;
    }
    break;
    case PATH:
    {
        drawTileCmd->m_color = TILE_PATH_COLOR;
    }
    break;
    case PATH_END:
    {
        drawTileCmd->m_color = TILE_PATH_END_COLOR;
    }
    break;
    case PATH_TRAVERSED:
    {
        drawTileCmd->m_color = TILE_PATH_TRAVERSED_COLOR;
    }
    break;
    default:
        drawTileCmd->m_color = TILE_EMPTY_COLOR;
    }

    drawTileCmd->m_pos = v2f{tile->m_pos.x, tile->m_pos.y};
    drawTileCmd->m_dim = v2f{tile->m_rect.m_dim.x, tile->m_rect.m_dim.y};
    return drawTileCmd;
}
...
void App::RenderCursor(RenderGroup *rg, v2f mousePos)
{
    ...
    mouseQuad->m_pos = mousePos;
    mouseQuad->m_color = COLORS::WHITE;
    mouseQuad->m_dim = v2f{.1f, .1f};
}
...
void Path::PushPath(RenderGroup *rg)
{
    ...
    while (currentTile != m_end)
    {
        ...
        drawLineCmd->m_start = currentTile->m_tile->m_pos;
        drawLineCmd->m_end = currentTile->m_next->m_tile->m_pos;
        drawLineCmd->m_colorStart = COLORS::RED;
        drawLineCmd->m_colorEnd = COLORS::GREEN;
    ...
    for (s32 y = 0; y < (s32)m_tilesPerHeight; ++y)
    {
        for (s32 x = 0; x < (s32)m_tilesPerWidth; ++x)
        {
            ...
            *tile = {};
            ...
            tile->m_rect = {};
            tile->m_rect.m_dim = v2f{NavMesh::TILE_WIDTH_METERS,
...
void NavMesh::Render(App *app, RenderGroup *rg)
{
    ...
    for (u32 i = 0; i < m_tilesPerWidth * m_tilesPerHeight; ++i)
    {
        ...
        for (int j = 0; j < NEIGHBOUR_COUNT; ++j)
        {
            ...
            dl->m_start = tile->m_pos;
            dl->m_end = link->m_target->m_pos;
            dl->m_colorStart = COLORS::BLUE;
            dl->m_colorEnd = COLORS::RED;
        }
    }
}
...
Entity *App::CreatePlayerEntity(v2f pos, v4f color)
{
    ...
    result->m_rect.m_dim = v2f{NavMesh::TILE_WIDTH_METERS, NavMesh::TILE_HEIGHT_METERS};
...
void App::Initialize(void *memory, MemoryMarker size)
{
...
void App::Update(const f32 deltaTime)
{
    ...
    clearScreenCmd->m_color = COLORS::BLACK;
    ...
    dl->m_start = v2f{4.0f, 4.0f};
    dl->m_end.x = oldTarget.x * cos(sAngle) - oldTarget.y * sin(sAngle);
    dl->m_end.y = oldTarget.x * sin(sAngle) + oldTarget.y * cos(sAngle);
    oldTarget = dl->m_end;
    dl->m_end += dl->m_start;
    dl->m_colorStart = COLORS::RED;
    dl->m_colorEnd = COLORS::GREEN;

    // NOTE(pf): Grid pattern
    for (u32 y = 0; y < m_navMesh.m_tilesPerHeight; ++y)
    {
        RCDrawLine *gridLine = renderGroup->PushCommand<RCDrawLine>();
        gridLine->m_start = v2f{0.0f,
                                y * NavMesh::TILE_WIDTH_METERS};
        gridLine->m_end = v2f{(m_navMesh.m_tilesPerWidth - 1) * NavMesh::TILE_WIDTH_METERS,
                              y * NavMesh::TILE_WIDTH_METERS};
        gridLine->m_colorStart = COLORS::WHITE;
        gridLine->m_colorEnd = COLORS::WHITE;
    }

    for (u32 x = 0; x < m_navMesh.m_tilesPerWidth; ++x)
    {
        RCDrawLine *gridLine = renderGroup->PushCommand<RCDrawLine>();
        gridLine->m_start = v2f{x * NavMesh::TILE_WIDTH_METERS,
                                0.0f};
        gridLine->m_end = v2f{x * NavMesh::TILE_WIDTH_METERS,
    ...
    for (MemoryPool<Entity>::MemPtr *it = (MemoryPool<Entity>::MemPtr *)m_firstEntity;;)
    {
        ...
        if (activeEntity->m_type == PLAYER)
        {
            ...
            highlightCmd->m_color = gSelectedEntity == activeEntity ? COLORS::GREEN : COLORS::RED;
            highlightCmd->m_pos = activeEntity->m_pos;
            highlightCmd->m_dim = activeEntity->m_rect.m_dim + v2f{0.05f, 0.05f};
        }
        ...
        entityRndCmd->m_color = activeEntity->m_color;
        entityRndCmd->m_pos = activeEntity->m_pos;
        entityRndCmd->m_dim = activeEntity->m_rect.m_dim;
...
GAME_UPDATE(GameUpdate)
{
    ...
    if (platformState->reloadedDLL)
    {
        ...
        else
        {
            ...
            app->Initialize(memory, platformState->memorySize - sizeof(App));
        }
    }

    if(platformState->input->KeyPressed(KEY::ESCAPE))
    {
